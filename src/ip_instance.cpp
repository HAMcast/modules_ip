...
{
    HC_LOG_TRACE("");
    std::uint32_t pipe_msg = s_queue_event;
...
void ip_instance::init()
{
    ...
    m_loop = std::thread(ip_instance::receive_loop_helper, this);
}
...
{
    HC_LOG_TRACE("");
    std::uint32_t pipe_msg = s_quit_event;
    if (write(write_handle(), &pipe_msg, sizeof(pipe_msg)) < 0)
...
uri ip_instance::map(const uri& group_uri)
{
     ...
     */
    string tmp_scheme;
    string uri_scheme = group_uri.scheme();
    std::transform(uri_scheme.begin(),uri_scheme.end(),tmp_scheme.begin(),toupper);
